...
public class HandgunScriptLPFP : MonoBehaviour
{
    ...
    {
        // Leapmotion_guesture.isShoot
        //Shooting 
        // if (Input.GetMouseButtonDown(0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning)
        if ((_isShoot || Input.GetMouseButtonDown(0)) && !outOfAmmo && !isReloading)
        {
            _isShoot = false;
            ...
            //cameraShake.VibrateForTime(2.0f);
        }

        //Reload 
        // if (Input.GetKeyDown(KeyCode.R) && !isReloading && !isInspecting)
        else if ((_isLoading || Input.GetKeyDown(KeyCode.R)) && !isReloading)
        {
            _isLoading = false;
            Reload();
        ...
        else if (_isGrenade || Input.GetKeyDown(KeyCode.G))
        {
            _isGrenade = false;
    ...
    private Vector3 CastRay()
    {
        ...
        RaycastHit hit;
        Ray ray = gunCamera.ScreenPointToRay(new Vector3(x, y));

        int _layerMask = 1 << LayerMask.NameToLayer("Enemy");
    ...
    private void NumOfAmmo()
    {
        ...
        else
        {
            ...
            currentWeaponText.text = storedWeaponName; // storedWeaponName.ToString(); (error)
            //Toggle bool
            outOfAmmo = false;
